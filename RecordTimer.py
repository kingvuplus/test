...
######### the following static methods and members are only in use when the box is in (soft) standby
		...
		if event == iRecordableService.evEnd:
			...
			if not recordings: # no more recordings exist
				...
				if rec_time > 0 and (rec_time - time()) < 360:
					...
					if not Screens.Standby.inTryQuitMainloop and config.usage.silent_rec_mode.value:
						print "[SILENT_REC] but we are not in QuitMainLoop --> we do not power off"
					else:
	...
	def TryQuitMainloop(default_yes = True):
		if not RecordTimerEntry.receiveRecordEvents:
			...
			Notifications.AddNotificationWithID("goStandby", Screens.Standby.TryQuitMainloop, 1, onSessionOpenCallback=RecordTimerEntry.stopTryQuitMainloop, default_yes = False)
#################################################################

	def __init__(self, serviceref, begin, end, name, description, eit, disabled = False, justplay = False, afterEvent = AFTEREVENT.AUTO, checkOldTimers = False, dirname = None, tags = None, zapbeforerecord = False, justremind = False):
		...
		self.tags = tags or []
		self.zapbeforerecord = zapbeforerecord
		...
		if self.name:
			if config.usage.setup_level.index >= 2: # expert+
				if config.recording.filename_composition.value == "short":
					...
					if config.usage.record_file_name_date_at_end.value:
						filename = self.name + " - " + begin_shortdate
				elif config.recording.filename_composition.value == "minimal":
					filename = self.name
				elif config.recording.filename_composition.value == "long":
					if config.usage.record_file_name_date_at_end.value:
						filename = self.name + " - " + service_name + " - " + self.description + " - " + begin_date
					else:
						filename += " - " + self.name + " - " + self.description
				else:
					if config.usage.record_file_name_date_at_end.value:
						filename = self.name + " - " + service_name + " - " + begin_date
					else:
	...
	def tryPrepare(self):
		if self.justplay or self.justremind:
			return True
		else:
	...
	def activate(self):
		...
		if next_state == 1 and (self.zapbeforerecord or config.usage.zap_before_record.value):
			cur_ref_for_zap = NavigationInstance.instance.getCurrentlyPlayingServiceReference()
			if cur_ref_for_zap and not cur_ref_for_zap.getPath(): # we do not zap away if it is no live service
				if config.usage.zap_notification_record.value == True:
					Notifications.AddNotification(MessageBox, _("In order to record a timer, the TV was switched to the recording service!\n"), type=MessageBox.TYPE_INFO, timeout=20)
				NavigationInstance.instance.playService(self.service_ref.ref)
				self.log(5, "zap to recording service to activate prioritized tuner")
		...
		if next_state == self.StatePrepared:
			if self.tryPrepare():
				...
				if not (self.justplay or self.justremind):
			...
			if self.justplay:
				if Screens.Standby.inStandby:
					...
					NavigationInstance.instance.playService(self.service_ref.ref)
				return True
			elif self.justremind:
				self.log(11, "show reminder")
				service_name = self.service_ref.getServiceName()
				msg = _("Reminder !\nFollowing preferred service starts now :\n\n%s\n%s\n\nDo you want to zap ?") % (service_name, self.name)
				Notifications.AddNotificationWithCallback(self.zaptoReminderCB, MessageBox, msg, timeout = 0, default = False)
				return True
			...
			if self.afterEvent == AFTEREVENT.STANDBY:
				...
				if config.usage.silent_rec_mode.value:
					print "[SILENT_REC] we are in auto mode"
	...
	def setAutoincreaseEnd(self, entry = None):
		...
		dummyentry = RecordTimerEntry(self.service_ref, self.begin, new_end, self.name, self.description, self.eit, disabled=True, justplay = self.justplay, afterEvent = self.afterEvent, dirname = self.dirname, tags = self.tags, zapbeforerecord = self.zapbeforerecord, justremind = self.justremind)
	...
	def zaptoReminderCB(self, answer):
		if answer:
			cur_ref_for_zap = NavigationInstance.instance.getCurrentlyPlayingServiceReference()
			if cur_ref_for_zap and not cur_ref_for_zap.getPath():
				NavigationInstance.instance.playService(self.service_ref.ref)
				self.log(12, "zap to service of reminder")
			else:
				msg = _("You have to stop your playing Movie before")
				Notifications.AddNotification(MessageBox, msg,type=MessageBox.TYPE_INFO, timeout = 20)
...
def createTimer(xml):
	...
	zapbeforerecord = long(xml.get("zapbeforerecord") or "0")
	justremind = long(xml.get("justremind") or "0")
	eit = xml.get("eit")
	if eit and eit != "None":
		eit = long(eit);
	else:
	...
	entry = RecordTimerEntry(serviceref, begin, end, name, description, eit, disabled, justplay, afterevent, dirname = location, tags = tags, zapbeforerecord = zapbeforerecord, justremind = justremind)
...
class RecordTimer(timer.Timer):
	...
	def saveTimer(self):
		...
		for timer in self.timer_list + self.processed_timers:
			...
			list.append(' zapbeforerecord="' + str(int(timer.zapbeforerecord)) + '"')
			list.append(' justremind="' + str(int(timer.justremind)) + '"')
			list.append('>\n')
	...
	def getNextRecordingTime(self):
		...
		for timer in self.timer_list:
			...
			if timer.justplay or next_act < now or timer.justremind:
				continue
			return next_act
		return -1
	...
	def isNextRecordAfterEventActionAuto(self):
		...
		t = None
		for timer in self.timer_list:
			if timer.justplay or timer.begin < now or timer.justremind:
				continue
	...
	def removeEntry(self, entry):
		...
		# test if timer is really in list and is not removed before (e.g.deleting timer manually from list)
